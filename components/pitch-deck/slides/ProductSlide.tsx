...
export function ProductSlide({ title, body, bullets }: { title: string; body?: string; bullets?: string[] }) {
  return (
    <SlideCanvas orbs={generateAuroraOrbs("product")}>
      <div style={{ display: "flex", height: "100%", padding: "6% 8%", gap: 40, alignItems: "center" }}>
        <div style={{ flex: 1, background: "#fff", borderRadius: 12, padding: 16, boxShadow: "0 20px 60px rgba(0,0,0,0.4)", maxWidth: "44%" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12, paddingBottom: 10, borderBottom: "1px solid #e5e7eb" }}>
            {["#ef4444","#f59e0b","#22c55e"].map((c,i) => <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: c }} />)}
            <div style={{ flex: 1, height: 18, background: "#f3f4f6", borderRadius: 4, marginLeft: 8 }} />
          </div>
          {[1,2,3].map(i => (
          ...
          <div style={{ marginTop: 12, padding: "10px 16px", background: "#3b82f6", borderRadius: 8, color: "#fff", fontSize: 13, fontWeight: 600, textAlign: "center" }}>Get Started →</div>
        </div>
        <div style={{ flex: 1 }}>
          <TitleLayer text={title} outlineSize="44px" solidSize="52px" />
