...
export function GenericSlide({ slideType, title, body, bullets, keystat }: { slideType: SlideType; title: string; body?: string; bullets?: string[]; keystat?: string }) {
  return (
    <SlideCanvas orbs={generateAuroraOrbs(slideType)}>
      <div style={{ padding: "6% 8%", height: "100%", display: "flex", flexDirection: "column" }}>
        ...
        {bullets && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 24 }}>
            {bullets.slice(0,4).map((b,i) => <LightCard key={i} body={b} />)}
          </div>
        )}
      </div>
    </SlideCanvas>
