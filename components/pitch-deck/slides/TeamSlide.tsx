...
export function TeamSlide({ title, team, body, bullets }: { title: string; team?: TeamMember[]; body?: string; bullets?: string[] }) {
  const members: TeamMember[] = team ?? bullets?.map(b => ({ name: b })) ?? (body ? [{ name: body }] : []);
  return (
    <SlideCanvas orbs={generateAuroraOrbs("team")}>
