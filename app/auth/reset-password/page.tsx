...
function ResetForm() {
  ...
  if (done) return (
    <div className="w-full max-w-sm bg-[#111111] border border-[#1a1a1a] rounded-lg p-8 text-center">
      ...
      <p className="text-sm text-[#888888] mb-6">Your password has been reset. You can now sign in.</p>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-sm">
      ...
      <div className="bg-[#111111] border border-[#1a1a1a] rounded-lg p-8">
        ...
        <p className="text-sm text-[#888888] mb-6">Choose a strong password for your account.</p>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="New Password" type={showPw ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 8 characters"
